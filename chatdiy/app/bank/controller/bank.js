mainChatApp.controller('bankController', ['$scope', '$rootScope', '$state', 'serverService', function ($scope, $rootScope, $state, serverService) {
			...
			$scope.bankName = function (type) {
				if ($rootScope.formData.fields.bankID !== '' && $rootScope.formData.fields.bankID !== null) {
					...
					if ($rootScope.formData.fields.bankName != "Axis Bank" && type == 'changed') {
						setTimeout(function () {
							$('#myModal').modal({
								backdrop: 'static',
								keyboard: false
					...
					} else if ($rootScope.formData.fields.bankName == "Axis Bank") {
			...
			$scope.bankValidate = function () {
				...
				if ($rootScope.formData.fields.ifscCode == null || $rootScope.formData.fields.ifscCode == '' || $rootScope.formData.fields.ifscCode.length != 11 || $rootScope.formData.ifscError) {
			...
			$scope.getFinData = function () {
				...
				$rootScope.formData.fields.accNumber = $scope.finData.officeDetails[0].AccountId;
				$rootScope.formData.fields.raccNumber = $scope.finData.officeDetails[0].AccountId;
				$rootScope.formData.fields.bankID = '1';
				...
				$scope.typePasswordCAC = true
					...
					setTimeout(function () {
						...
						$('#acc1').val($scope.finData.officeDetails[0].AccountId)
						$('#cacc1').val($scope.finData.officeDetails[0].AccountId)
						$(".select").select2();
