mainApp.controller('rmController', ['$scope', '$rootScope', '$state', 'serverService', '$location', function ($scope, $rootScope, $state, serverService, $location) {
	...
	$scope.generatePOA = function (ref, type) {
		...
		serverService.apiCall(url, sendData).then(function (a) {
			...
			$rootScope.formData.apiLoading = false;
			if (response.IsSuccess) {
				var rejectModalPop = new bootstrap.Modal(document.getElementById('rejectModal'));
				rejectModalPop.show();
