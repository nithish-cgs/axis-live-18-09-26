...
mainApp.controller('documentController', ['$scope', '$rootScope', '$state', 'serverService', '$window', '$location', '$interval', function ($scope, $rootScope, $state, serverService, $window, $location, $interval) {
									...
									if (arr.length > 0) {
										...
										nominees.forEach((nominee, index) => {
											let nomineeFields = [
												...
												{ field: "Mobile Number", disabled: nominee.Mobile ? true : false, value: nominee.Mobile || '', required: true, type: 'input', directive: 'digit', length: 10 },
												{ field: "Email ID", disabled: nominee.Email ? true : false, value: nominee.Email || '', required: true, type: 'input', directive: 'email' },
												{
													field: "Id Proof", disabled: nominee.NomineeProofId ? true : false, value: nominee.NomineeProofId ? nominee.NomineeProofId.toString() : '', required: true, type: 'dropdown',
											...
											if (nominee.IsGuardian === "1") {
												let guardianFields = [
													...
													{ field: "Guardian Mobile Number", readonly: nominee.GuardianMobile ? true : false, value: nominee.GuardianMobile || '', required: true, type: 'input', directive: 'digit', length: 10 },
													{ field: "Guardian Email ID", readonly: nominee.GuardianEmail ? true : false, value: nominee.GuardianEmail || '', required: true, type: 'input', directive: 'email' },
													{
														field: "Guardian Id Proof", readonly: nominee.GuardianProofId ? true : false, value: nominee.GuardianProofId ? nominee.GuardianProofId.toString() : '', required: true, type: 'dropdown',
														arrayVal: $rootScope.formData.newNomineeTypeList, labelKey: 'Name',
														valueKey: 'DocumentTypeId'
													},
													{
			...
			$scope.displayForm = function () {
				...
				if ($rootScope.BYOD && !$scope.vcipDigi) {
					...
					if (!$scope.documentChangeNumber &&	!$scope.byodnumberValidation && !$rootScope.formData.digiData) {
						$scope.showValidateFields = true;
						$scope.$evalAsync();
						error++;
					}
			...
			$rootScope.nomineeNewFields.forEach((fields, index) => {
				...
				fields.forEach((data) => {
					...
					if (data.field === "sameAddress") {
						obj.IsNomineeSameAsApplicant = data.value ? '1' : '0';
					}
