mainApp.controller('personalController', ['$scope', '$rootScope', '$state', 'serverService', function ($scope, $rootScope, $state, serverService) {
	...
	$scope.nYes = function (value) {
		...
		if ($rootScope.formData.fields.nominee == 'Y') {
			...
			//if ($rootScope.nomineeNewFields.length > 0) {
				...
				NomineDeletePopUpModal.show(); //new popup.
			//}
		}
	}
		...
		serverService.apiCall(s_url, sendData).then(function (a) {
			...
			if (response.IsSuccess) {
				if (response.IsSuccess) {
					...
					//if (response.IpvPOAandNomineeList.length > 0) { //backup
						...
						if (arr.length > 0) {
							...
							nominees.forEach((nominee, index) => {
								let nomineeFields = [
									...
									{
										field: "Relationship with Minor", value: nominee.RelationshipWithNominee ? nominee.RelationshipWithNominee.toString() : '', required: true, type: 'dropdown',
	...
	$scope.cancelDelete = function () {
		$rootScope.formData.fields.nominee = ''; //sabari
		$rootScope.nomineePrint = ''; //sabari	
		$('#nYes').prop('checked', false); //sabari
		$('#nNo').prop('checked', false);  //sabari
		$scope.noNominee(); //sabari
		$scope.nomineeDeclCheck1 = false; //sabari
		$scope.nomineeDeclCheck2 = false; //sabari
		$scope.nomineeDeclCheck3 = false; //sabari
		//$rootScope.formData.fields.nominee = 'Y';
		setTimeout(function () {
			$scope.checkradioinit();
		}, 20);
	...
	$scope.confirmNomineeOptOut = function () {
		...
		var sendData = [{
			...
			NomineeFlag: $rootScope.formData.fields.nominee
		}];
		sendData = $rootScope.encryptReq(sendData);
		$rootScope.formData.apiLoading = true;
					...
					if ($rootScope.nomineeNewFields.length > 0) {
						...
						$rootScope.nomineeNewFields.forEach((fields, index) => {
							...
							fields.forEach((data) => {
								...
								if (data.field === "Relationship with Minor") {
